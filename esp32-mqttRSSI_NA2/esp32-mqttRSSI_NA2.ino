...
AutoConnectAux mqtt_save("/", "MQTT Setting", true, {
  style,
  caption2,
  parameters,
  parameters2,
...
// Retreive the value of each element entered by '/mqtt_setting'.
String saveParams2(AutoConnectAux& aux, PageArgument& args) {
  parameters2.value.trim();


  // Echo back saved parameters to AutoConnectAux page.
  String echo = "parameters2 CAP  : " + parameters2.value + "<br>";
...
void handleClearChannel() {
  ...
  WiFiClient  client;
  parameters.value = parameters2.value;
  Serial.println(parameters.value);
  Serial.println(parameters2.value);
  
  parameters2.value = "Ashique HUssain";
...
void setup() {
  ...
  portal.join({ mqtt_save });
  //portal.on(AUX_SAVE_URI, saveParams);
  //portal.on(AUX_CLEAR_URI, saveParams2);
