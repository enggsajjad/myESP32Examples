/**
 * modified by a teco.edu member to support display
 * 
 */
  ...
  if (!cs) {
    ...
    if (_apConfig.autoRise) {
      ...
      AC_DBG("SoftAP %s/%s Ch(%d) IP:%s %s\n", _apConfig.apid.c_str(), _apConfig.psk.c_str(), _apConfig.channel, _currentHostIP.toString().c_str(), _apConfig.hidden ? "hidden" : "");
        ...
        u8x8.clearDisplay();
        u8x8.drawString(0, 0, "BITTE           ");
        u8x8.drawString(0, 1, "SENSORKNOTEN    ");
        u8x8.drawString(0, 2, "EINRICHTEN      ");
        u8x8.drawString(0, 3, "UNTER           ");  
        u8x8.drawString(0, 4, "SENSOR-WIFI     ");  
        u8x8.drawString(0, 5, "SSID:");
        u8x8.drawString(6, 5, esp32_chipid.c_str());
        u8x8.drawString(0, 6, "PW: 12345678    ");  
        u8x8.drawString(0, 7, "IP:192.168.244.1");
...
wl_status_t AutoConnect::_waitForConnect(unsigned long timeout) {
  ...
  u8x8.drawString(0, 0, "SUCHE WIFI...   ");
