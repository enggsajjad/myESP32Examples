...
void Datastream::toJSONObject(JsonObject& root) { //old function for maintaining compatibility 
	...
	JsonObject unitOfMeasObject = root.createNestedObject("unitOfMeasurement");
	unitOfMeasurement.toJSONObject(unitOfMeasObject);
	
	JsonObject thingIdObject = root.createNestedObject("Thing");
	thingIdObject["@iot.id"] = thingId;

	JsonObject sensorIdObject = root.createNestedObject("Sensor");
	sensorIdObject["@iot.id"] = sensorId;

	JsonObject observedPropertyIdObject = root.createNestedObject("ObservedProperty");
